package frc.robot.subsystems.shooter;

public final class ShooterConstants {

  public static final class HoodConstants {

    public static final int hoodID = 0;
    public static final int hoodEncoderID = 0;

    public static final int hoodP = 0;
    public static final int hoodI = 0;
    public static final int hoodD = 0;
    public static final int hoodS = 0;
    public static final int hoodV = 0;
  }

  public static final class LauncherConstants {

    public static final int launcherID = 0;
    public static final int launcherEncoderID = 0;

    public static final int launcherP = 0;
    public static final int launcherI = 0;
    public static final int launcherD = 0;
    public static final int launcherS = 0;
    public static final int launcherV = 0;
  }

  public static final class TurretConstants {

    public static final int turretID = 0;
    public static final int turretEncoderID = 0;

    public static final int turretP = 0;
    public static final int turretI = 0;
    public static final int turretD = 0;
    public static final int turretS = 0;
    public static final int turretV = 0;
  }
}
